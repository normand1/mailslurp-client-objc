...
case $state in
  ops)
    ...
    _values "Operations" \
            ...
            "waitForLatestEmail[Fetch inbox's latest email or if empty wait for email to arrive]"             "bulkCreateInboxes[Bulk create Inboxes (email addresses)]" \
            ...
            "getEmails[List Emails in an Inbox / EmailAddress]" \
  ...
  args)
    case $line[1] in
      ...
      waitForLatestEmail)
        local -a _op_arguments
        _op_arguments=(
                    "inboxEmailAddress=:[QUERY] Email address of the inbox we are fetching emails from"
...
"emailId=:[PATH] emailId"
      ...
      getEmails)
        ...
        _op_arguments=(
          ...
          "limit=:[QUERY] Limit the result set, ordered by descending received date time"
